...
class Main(config: Config, paths: MainPaths) {
  ...
  def updatedTargetDir(projectName: String, repo: String): Future[TargetDirs] =
    Future {
      val projectFolder = paths.cacheFolder / projectName
      if (os.exists(projectFolder)) {
        ...
        Try {
          ...
          interfaceCmd.runVerbose git ('clone, repo)
        } recover {
          case _ =>
            ...
            interfaceCmd.runVerbose git ("remote", "add", "origin", repo)
        }
  ...
  def run(): Unit = {
    val compilerF: Future[BloopCompiler] =
      Future(
        BloopCompiler(logger.filter(LogLevel.debug).void, config.versions, ec, paths.cacheFolder / 'compileFailures),
      )(ec)
    ...
    val updatedTargetDirF = updatedTargetDir(config.projectName, config.repo)
    ...
    val Pipeline: RecPhase[Source, PublishedSbtProject] =
      RecPhase[Source]
        ...
        .next(
          new Phase2ToScalaJs(
            config.pedantic,
            enableScalaJsDefined = config.enableScalaJsDefined,
            outputPkg            = config.flavour.outputPkg,
          ),
          "scala.js",
        )
