...
/**
  ...
  */
class Phase3WriteFiles(targetFolder: os.Path, softWrites: Boolean)
    extends Phase[Source, Phase2Res, Map[Source, Seq[os.Path]]] {
    ...
    _lib match {
      case lib: LibScalaJs =>
        getDeps(lib.dependencies.keys.map(x => x: Source).to[SortedSet]) flatMap {
          depsWritten: SortedMap[Source, Map[Source, Seq[Path]]] =>
