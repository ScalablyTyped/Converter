  ...
  def apply(location: JsLocation, isWithinScalaModule: Boolean): Seq[ClassAnnotation] =
    if (isWithinScalaModule) Seq(JsNative)
      ...
      location match {
        ...
        case JsLocation.Global(jsPath) => Seq(JsGlobal(ImportName.skipConversion(jsPath)), JsNative)
        case JsLocation.Module(lit, spec) =>
          spec match {
            ...
            case ModuleSpec.Specified(idents) =>
              ...
              Seq(JsImport(parts.mkString("/"), Imported.Named(ImportName.skipConversion(idents.last))), JsNative)
          }
      }
}
