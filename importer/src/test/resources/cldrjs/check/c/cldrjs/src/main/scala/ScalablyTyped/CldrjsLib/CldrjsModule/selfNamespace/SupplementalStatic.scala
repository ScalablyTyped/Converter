package ScalablyTyped
package CldrjsLib.CldrjsModule.selfNamespace

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait SupplementalStatic extends js.Object {
  val timeData: CldrjsLib.CldrjsCldrDotJsDashSupplementalModule.CldrjsModule.TimeDataStatic = js.native
  val weekData: CldrjsLib.CldrjsCldrDotJsDashSupplementalModule.CldrjsModule.WeekDataStatic = js.native
  def apply(path: java.lang.String): js.Any = js.native
  def apply(paths: StdLib.Array[java.lang.String]): js.Any = js.native
}

