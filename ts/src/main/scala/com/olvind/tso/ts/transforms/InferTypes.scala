      ...
      case (_, TsFunSig(_, _, params, None), Some(owner)) =>
        ...
        rewrittenOpt match {
          case Some(rewritten) =>
            ...
            rewritten
          case None =>
            scope.logger.warn(s"Could not infer return type")
            x
        }
      case _ => x
    }
