...
object ScalaJsReactComponents {
  ...
  def ScalaJsReactElement(isSvg: Boolean, name: String): TypeRef =
    TypeRef.Singleton(
      TypeRef(
        (if (isSvg) scalaJsReact.japgollyScalajsWebSvg else scalaJsReact.japgollyScalajsWebHtml) + Name(name) + Name("tag"),
        Nil,
        NoComments,
      ),
    )
  ...
  def apply(_scope: TreeScope, tree: ContainerTree): ContainerTree = {
                ...
                MethodTree(
                  ...
                  impl = MemberImpl.Custom(
                    s"""{
                       ...
                       |  val f = JsForwardRefComponent.force[Props, ${if(hasChildren) "Children.Varargs" else "Children.None"}, ${c.name.value}Type](js.constructorOf[${c.name.value}Type])
                       |
    ...
    scalaJsReactMembers match {
      case Nil => tree
      case nonEmpty =>
        tree.withMembers(tree.members :+ PackageTree(Nil, scalaJsReact.ScalaJsReact, nonEmpty, NoComments, scalaJsReactModCp))
    }
  }
