package typingsJapgolly.stardustUiReactComponentEventListener

import typingsJapgolly.stardustUiReactComponentEventListener.typesMod.EventListenerOptions
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object useEventListenerMod {
  
  @JSImport("@stardust-ui/react-component-event-listener/dist/es/useEventListener", JSImport.Namespace)
  @js.native
  val ^ : js.Any = js.native
  
  inline def default[T /* <: /* import warning: LimitUnionLength.leaveTypeRef Was union type with length 91 */ String */](options: EventListenerOptions[T]): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("default")(options.asInstanceOf[js.Any]).asInstanceOf[Unit]
}
